...
namespace Rebirth.World.Datas.Characters
{
    public class CharacterInfos
    {
        ...
        public double LastConnexion { get; set; }

        public byte Level { get { return ExperienceManager.Instance.GetCharacterLevel((long)Experience); } }
        public long NextExp { get { return ExperienceManager.Instance.GetCharacterNextLevelExperience((byte)(Level)); } }
        #endregion
        ...
        public CharacterInfos(double id, byte[] datas)
        {
            ...
            LastConnexion = reader.ReadDouble();
        }
        #endregion
        ...
        public byte[] GetDatas()
        {
            ...
            writer.WriteDouble(LastConnexion);
            return writer.Data;
        }

        public string[] LastConnexionForGame()
        {
            var date = LastConnexion.UnixTimestampToDateTime();
            return new string[] { date.Year.ToString(), date.Month.ToString(), date.Day.ToString(), date.Hour.ToString(), date.Minute.ToString(), "127.0.0.1" };
        }
        #endregion
