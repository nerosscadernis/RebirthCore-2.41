    ...
    public class CharacterFrame
    {
        ...
        private void HandleCharacterCreationRequestMessage(Client client, CharacterCreationRequestMessage msg)
        {
            ...
            var character = new Character()
            {
                ...
                Spells = new SpellsInfo(),
            };
            character.Jobs = new JobInfos(character);
            character.Inventory = new Inventaire(character);
            character.Quests = new QuestInfos(character);
            character.Stats = new StatsFields(character);
            character.Shortcuts = new ShortcutBar(character);
            character.Stats.Initialize();

            character.Stats[PlayerFields.Strength].Base += 500;

            character.Spells.AddSpell((int)0);
            character.Shortcuts.AddShortcut(ShortcutBarEnum.SPELL_SHORTCUT_BAR, new ShortcutSpell((sbyte)character.Shortcuts.GetNextFreeSlot(ShortcutBarEnum.SPELL_SHORTCUT_BAR), 0));
            for (int i = 0; i < breed.breedSpellsId.Count; i++)
            {
                var id = breed.breedSpellsId[i];
                character.Spells.AddSpell((int)id);
                character.Shortcuts.AddShortcut(ShortcutBarEnum.SPELL_SHORTCUT_BAR, new ShortcutSpell((sbyte)character.Shortcuts.GetNextFreeSlot(ShortcutBarEnum.SPELL_SHORTCUT_BAR), id));
            }
