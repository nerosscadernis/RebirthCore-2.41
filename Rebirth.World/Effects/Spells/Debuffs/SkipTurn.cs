...
namespace Rebirth.World.Game.Effect.Spells.Buffs
{
    ...
    {
        public SkipTurn(EffectDice effect, Fighter caster, SpellTemplate spell, CellMap targetedCell, bool critical) : base(effect, caster, spell, targetedCell, critical)
        {
        }
        public override bool Apply(object token, bool declanched = false)
        {
            ...
            foreach (Fighter current in base.GetAffectedActors())
            {
                ...
                SkipTurnBuff buff = new SkipTurnBuff(current.PopNextBuffId(), current, Caster, Dice, Spell, Critical, false, declanched);
