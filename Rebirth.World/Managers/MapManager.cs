...
namespace Rebirth.World.Managers
{
    public class MapManager : DataManager<MapManager>
    {
        ...
        {
            var maps = ObjectDataManager.GetAll<MapPosition>(v => v.posX == x && v.posY == y);
        ...
        public Dictionary<MapNeighbour, int> GetAlternative(int mapId)
        {
            return Starter.Database.Fetch<MapAlternative>("SELECT * FROM maps_alternative Where Enter='{0}'", mapId).ToDictionary(x => (MapNeighbour)x.Direction, x => x.Exit);
        }
